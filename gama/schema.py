...
class ErrorDetail(BaseModel):
    file: Optional[str] = Field(
        None, description="Path to the file where the error was found."
    )
    line: Optional[int] = Field(
        None, description="Line number where the error was found."
    )
...
class EvaluatorResult(BaseModel):
    ...
    errors: List[ErrorDetail] = Field(
        default_factory=list, description="List of errors found during evaluation."
    )
...
class EvaluatorErrorAggregation(BaseModel):
    ...
    errors: List[ErrorDetail] = Field(
        ..., description="Errors found by this evaluator."
    )
...
class StateContext(BaseModel):
    ...
    evaluator_results: Dict[str, EvaluatorResult] = Field(
        ..., description="Results per evaluator."
    )
    aggregated_errors: List[EvaluatorErrorAggregation] = Field(
        default_factory=list, description="All errors aggregated by evaluator."
    )
    overall_status: str = Field(
        ..., description="Overall status (e.g., 'PASSED', 'FAILED', 'NO_EVALUATORS')."
    )
