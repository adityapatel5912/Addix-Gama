        ...
        for path in target_dir.rglob("*"):
            ...
            if path.is_file() and path.suffix in extensions_to_check:
                try:
                    ...
                    redirect_uri_pattern = re.compile(
                        r"""redirect_uri\s*[:=]\s*['"](http://(?!(localhost|127\.0\.0\.1))[^'"]+)['"]""",
                        re.IGNORECASE,
                    )
                    ...
                    query_redirect_pattern = re.compile(
                        r"""redirect_uri=http://(?!(localhost|127\.0\.0\.1))[^&\s'"]+""",
                        re.IGNORECASE,
                    )
                    ...
                    for match in oauth_url_pattern.finditer(content):
                        url = match.group(1)
                        if "client_id=" in url and (
                            "response_type=" in url or "/authorize" in url
                        ):
                            if "state=" not in url:
                                errors.append(
                                    ErrorDetail(
